...
In order to satisfy this test, the following must be satisfied:
...
Within this test, the range for `b_exp` is the same as the ranges determined by `a_exp` in Test 1 and the range for `a_exp` is the same as `b_exp` in Test 1.

For simplicity, this test is accomplished with the same logic as Test 1. However, when generating the test vectors, `b_exp` and `a_exp` are swapped.
